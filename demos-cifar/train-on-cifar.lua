...
cmd:option('-optimization', 'SGD', 'optimization method: SGD | ASGD | CG | LBFGS | SDProp | Adam | RMSProp ')
...
function train(dataset)
   ...
   for t = 1,dataset:size(),opt.batchSize do
      ...
      elseif opt.optimization == 'SDProp' then
         config = config or {}
         optim.sdprop(feval, parameters, config)
